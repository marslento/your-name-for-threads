...
Last updated: 2026-09-19. This applies to Your Name for Threads 1.0, which has not been published yet.
...
The extension contains no code that makes network requests of its own, and it asks for access to `https://www.threads.com/*` only. No data is sold, and the extension shares none with the developer or with any third party.

Data leaves your browser only when you do something with it: export a backup file, copy diagnostics, or paste either one somewhere, such as a public GitHub issue.
...
| The username and numeric ID of accounts the Threads page shows you, whether or not you nicknamed them (the identity cache) | To recognise an account before you have nicknamed anyone, and to confirm which account is signed in. It is shared by every Threads account in this browser profile. | An entry counts as expired after 30 days, but it is only deleted when the same username is looked up again. Nothing removes the rest, and there is no way to clear the cache from the extension. Removing the extension deletes it. |
...
- **Export recovery data**, or **clear this account's data**, from the Recovery page, if the extension tells you that your data needs recovery. Export first: clearing deletes the damaged data for good.
...
- **Export a backup** regularly, so you do not lose your data if the browser's data is cleared.
