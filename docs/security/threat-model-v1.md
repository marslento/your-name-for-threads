...
| Files the person opens | Nothing | A backup file, or a single-account Recovery file, whoever made it | Text that must be parsed and checked before anything is written. |
...
- The viewer is read from one named define in the page's bootstrap data, never found by scanning for something that looks like a user. This avoids mistaking an ordinary post author, profile header or account-switcher entry for the viewer (`src/account/viewerEvidence.ts`). The data is read from the shared DOM; its server-rendered origin is not authenticated.
- There are three outcomes, not two: confirmed; explicitly unresolved (a null viewer, a malformed one, defines that disagree, an unreadable blob); and unavailable. A null or malformed viewer stays unresolved even alongside a valid viewer. An explicit "no viewer" ends the resolver's retries at once; "unavailable" is read again a few times, then left unresolved.
- Nothing else stands in for that define. The navigation's profile link, resolved through the identity cache, once did; the cache is fed by page messages any script can forge (T9) and written by several tabs at once, so the fallback was removed and the cache is used for display only (`tests/account/owner-evidence.test.tsx`).
- No confirmed owner means no private Directory access, and there is no grace period. A tab's proof belongs to its current document, which the browser names (`sender.documentId`): a report is recorded against that document, and a report still in flight from a document the tab has since replaced is dropped whole, so it can neither confirm an account nor overwrite the newer page's (`recordTabReport` in `src/account/AccountContextRegistry.ts`). That holds for a page's first report too: a page that leaves before it lands is remembered as gone even though it had no record yet.
- Once a document is retired, its ID stays in the tab's session record until the tab closes; no fixed-length history evicts older IDs. The history survives background worker restarts. Regression tests replay retired confirmations and loss-of-proof reports after twenty replacements and verify that they cannot change the current account or its Dashboard.
...
**Evidence.** `tests/account/viewer-evidence.test.ts`, `tests/account/threads-account-resolver.test.ts`, `tests/account/owner-evidence.test.tsx`, `tests/account/revalidation-state-machine.test.ts`, `tests/account/account-context-registry.test.ts`, `tests/account/account-write-authority.test.ts`, `tests/storage/account-write-cancellation.test.ts`, `tests/background/serviceWorker.test.ts`, `tests/background/dashboardLifecycle.test.ts`, `e2e/dashboard-lifecycle.spec.ts`.

**Residual.**

- Every piece of evidence is supplied by the page. A hostile script running inside the Threads page (Threads' own, or injected) could forge the bootstrap blob, and the extension could not tell. This is an analysis of the code, not a tested attack. The script could make the content script act for another account whose Directory is in this browser profile, read that account's rendered nicknames, and operate the profile controls to edit nicknames or delete contacts. Notes are not rendered or readable through the normal page interface, but deleting a contact also deletes its note. The Dashboard is an extension page the script cannot read. This is a non-goal (below).
...
- Retirement history grows with document replacements until the tab closes and shares the browser's session-storage quota. Retaining it closes the known eviction/replay gap; it does not establish that Chrome can deliver a report twenty documents late, or identify a stale document that was never recorded as retired.
...
- An imported backup may carry at most 20,000 records, contacts and deleted records together (`MAX_DIRECTORY_RECORDS`). A file with more is refused before the schema reads any entry, so a small file of many tiny bad entries cannot make validation keep an error for each one. A schema check that throws instead of answering, which zod does once the lists hold enough bad fields, refuses the file the same way.
- Export preserves the complete backup, including existing data above the record limit. If the export exceeds 20,000 combined records or 10 MiB, the export and the backup step before Clear or Restore show a persistent warning that the file cannot be imported directly. Downloading it does not grant an exception to either import limit.
- A nickname or note is counted only to one past its limit, so one enormous field costs no more than a short one (`tests/domain/validation.test.ts`).
...
- Imported record IDs reject the exact keys `__proto__`, `constructor` and `prototype`, both when parsing the backup and when validating the candidate before writing. Other nonblank legacy IDs remain supported. Storage readers and rebuilders preserve existing records with special keys as own entries, so loading them does not lose a Directory or its account binding (`tests/portability/storage-key-safety.test.ts`, `tests/storage/v1-to-v2-migration.test.ts`).
- The backup is normalised, then checked for cross-record invariants (duplicate contact or tombstone IDs, duplicate active usernames, an ID that is both active and deleted, a merge that points nowhere).
...
- External import review resolves the complete incoming contact ID, including colons and whitespace, so the preview and the saved decision refer to the same record. The review list uses pages of 25, 50 or 100 rows; switching pages does not discard decisions or truncate the backup.
- When an incoming username already belongs to an active contact, review does not offer to create a second contact with it. A weak identity match can be confirmed or kept local; a conflicting numeric identity can be kept local. The candidate validator checks uniqueness again at commit time.
...
**Evidence.** `tests/portability/parse-backup.test.ts`, `tests/domain/validation.test.ts`, `tests/storage/directoryAccess.test.ts`, `tests/portability/backup-schema.test.ts`, `tests/portability/validate-backup.test.ts`, `tests/portability/validate-candidate.test.ts`, `tests/portability/normalize-backup.test.ts`, `tests/portability/import-concurrency.test.ts`, `tests/dashboard/import-flow.test.tsx`, `tests/dashboard/import-review-binding.test.ts`, `tests/dashboard/ImportReviewList.test.tsx`, `src/portability/parseBackup.ts`, `tests/repo/threat-model.test.ts`.

**Residual.** A backup is unsigned plain JSON. A well-formed file that says false things is accepted, because nothing can prove who made it. The person sees a preview and, for a backup that names another account, an explicit confirmation (T5). Authenticity of a backup is a non-goal. Pagination bounds rendered rows; parsing and analysis still process the full file within the 10 MiB limit and can take time on slower devices. A Directory stops growing at 20,000 records: deleted records count and are kept, so deleting a contact does not make room; clearing the account's data, or restoring a smaller eligible backup, does. An exported file is not always restorable: existing oversized data remains fully downloadable, but a file above either import limit cannot be restored directly.
...
### T7. Recovery dump confusion and restore

**Threat.** A Recovery file is mistaken for a backup and imported over good data, a backup is mistaken for a Recovery file, or a dump that holds more than one account is shared. Since 1.1.0 a Recovery file can be restored, so also: a restore overwrites or merges into good data, lands in another account, trusts an edited or forged file, races another write, or reports a result it cannot confirm.

**Mitigations.**

- A Recovery file has its own name (`your-name-for-threads-recovery-...json`) and its own shape. The backup importer refuses it. The one reader is a separate entry point, "Restore from a recovery file" (`src/recovery/recoveryImport.ts`); the backup importer points a Recovery file there and reads it no further.
- For one damaged Directory the dump holds only that account's Directory record and its binding, exactly as stored. Only when the root of storage is damaged does it hold every account's data, and the Recovery page says so. Such a file of all storage is refused by the restore.
...
- Damaged Directories are set aside untouched, so exporting, clearing or restoring one later still sees it byte for byte (`src/recovery/quarantine.ts`).
- Validation rejects mismatched contact IDs, duplicate active usernames, identity indexes pointing to the wrong contact and malformed conflict fields before consumers use them. The original Directory is retained for recovery; other accounts remain usable (`tests/recovery/directory-identity-integrity.test.ts`). Rebuilding an identity index omits ambiguous username entries, and the contact cache also refuses ambiguous username lookups from storage change events (`tests/domain/identity-reconciliation.test.ts`, `tests/storage/contactStore.test.ts`).
- A Recovery file is unsigned input. The restore reads a single account's file only, and the binding in it is a consistency check against the signed-in account, never authority. The file is limited to 10 MiB, checked before it is read, and to 20,000 contacts and deleted records, counted before any record is checked. Every contact and deleted record must pass the backup's own strict schema and the final candidate checks, and one failure refuses the whole file: nothing is trimmed, re-dated, skipped or repaired. The lookup index and pending conflicts are rebuilt from the contacts, not trusted; contacts that share a stable ID get a new pending conflict, and none is picked.
- It never overwrites or merges. The target must be the account's truly empty Directory (no contacts, deleted records or pending conflicts), or the account's own damaged Directory still holding exactly the file's contacts and deleted records, compared value by value and never by date. A Directory ID used by any other Directory, set-aside and unbound ones included, refuses it (`src/recovery/recoveryTarget.ts`).
- Writing an account's own set-aside Directory is a deliberate exception, confined to this entry point and to the checks above. Ordinary writes still refuse every set-aside Directory.
- The write goes through the same queue and the cross-context lock, in a mode with no unlocked fallback. Under the lock storage is read again and every check repeated, including that this account's binding and Directory are exactly what the preview saw; the account authority captured before the file was read is checked again, with no wait between that check and the write. It is one `set` of `directories` and `accountBindings`, never a clear followed by an import, and every other Directory, binding and set-aside Directory is written back as it was read (`src/recovery/recoveryRestore.ts`, `src/storage/directoryLock.ts`).
- The previous way this damage arose is closed: a username match no longer replaces a contact's stored stable ID (`src/storage/BrowserStorageContactsRepository.ts`), and page observations do not write private contacts at all.

**Evidence.** `tests/recovery/recovery-export.test.ts` (the importer refuses a Recovery file, even one made to look more like a backup), `tests/recovery/damaged-directory-isolation.test.ts`, `tests/recovery/recovery-page.test.tsx`, `tests/recovery/recovery-import.test.ts`, `tests/recovery/recovery-target.test.ts`, `tests/recovery/recovery-restore.test.ts`, `tests/recovery/recovery-roundtrip.test.ts`, `tests/recovery/recovery-import-card.test.tsx`, `tests/recovery/dashboard-recovery.test.tsx`, `tests/storage/contactsRepository.test.ts`.

**Residual.** The file is unchecked and unencrypted when it is saved, and may hold private nicknames, notes and IDs. Once saved it is the person's to protect. A restore is not a transaction: once the write is sent it is read back, and a result that cannot be read back is reported as unknown, never as success, and is never rolled back; a retry recognises a restore that already landed, and never sends the same restore's write a second time: if what is stored is not exactly that restore, the file has to be checked again. A file with any record that fails is not restored at all: there is no partial salvage, no restore of a file of all storage, and no merge into a Directory that has data. A write that does not go through the extension's lock is not guarded against. Rebuilding the index does not tell which Threads ID is the right one: each contact keeps the ID it stored.
...
- Nickname labels carry no internal contact UUID. Their ownership marker is sufficient for lookup and removal (`tests/post-author/renderer.test.ts`).
- What it removes is what it added: nodes it holds a reference to, or nodes carrying its own marks. Turning the extension off sweeps every rendered label by its mark. Losing the owner removes every TPD-owned surface node before anything else happens. A label goes together with the separator text inserted beside it, which the extension holds a reference to, so the post is left as Threads drew it and the next owner's label can be drawn (`tests/post-author/PostAuthorSurfaceAdapter.test.tsx`, `tests/runtime/performance-contract.test.tsx`).
...
**Evidence.** `tests/runtime/threadsRuntime.test.ts`, `tests/post-author/PostAuthorSurfaceAdapter.test.tsx`, `tests/runtime/performance-contract.test.tsx`, `tests/profile/profileShadow.test.tsx`, `src/content/ui/display/NicknameLabelRenderer.ts`.

**Residual.** Ownership is decided by a marker on a node. A page could mark one of its own nodes, and the extension would remove it, which harms only the page. The page can equally remove or change the extension's nodes, so what is shown on Threads is not protected from the page. Both Shadow Roots are open (`mode: "open"`), so they separate styles and not authority: a script in the page can read the profile interface and operate it, including its save and delete buttons. Deleting a contact also deletes its note, although the note is not shown in the page interface. Apart from the nickname-label test above, which compares posts before and after a sweep with Threads' own separator in them, no test plants an unmarked Threads node and checks that a sweep leaves it; that is read from the selectors, not proved.
...
- These checks do not authenticate the sender. Accepted observations update the shared public identity cache and refresh display; the coordinator cannot attach or replace a saved contact's numeric ID, rename it or create a private identity conflict. Tests send forged messages through the real bridge and repository and verify that the Directories and account bindings stay unchanged (`tests/identity/identityCoordinator.test.ts`, `tests/contentBootstrap.test.tsx`).
...
- If loading settings fails, the profile interface, page runtime and observer stay disabled, even when contacts loaded successfully through a separate read. Reloading the page retries startup; a failed settings read does not enable defaults (`tests/contentBootstrap.test.tsx`).
...
- The channel is a same-window `postMessage`, so any script in the page can send the same messages: it can post false identity observations, and it can send the message that switches the observer on or off. The content script cannot tell an observation from the observer from one from another script. False observations can poison the persisted public cache and affect nickname display; they cannot choose the account (T2). They no longer automatically change saved contacts or create private conflicts. Explicit saves and imports remain separate write paths; a new contact saved by the person may use a cached identity.
...
- Observations never confirm an account owner, alone or through the cache (T2).
...
- An event can say only one of a closed set of codes, one of a closed set of components, a closed state, a timestamp and the extension version. There are 11 codes and 7 components in `src/diagnostics/diagnosticTypes.ts`. There is no free-text field.
...
- Every Action in the CI, browser-test, Pages and release workflows is pinned to a full commit SHA, with its corresponding version in a comment. Updating an Action requires changing and reviewing that pin.
...
- The final ZIP smoke test stops after any failed preflight check or preflight exception, before creating a certificate, starting its local server or loading the package into a browser (`scripts/verification/final-zip-smoke/run.mjs`, `tests/repo/final-zip-preflight.test.ts`).
